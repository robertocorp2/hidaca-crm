...
2. Apply the additive `drizzle/0022_prospecting_intelligence.sql` and `drizzle/0023_place_id_first_google_context.sql` migrations through the established Sites migration process after the earlier migrations. The build packages upward migrations only. `npm run db:generate` should report no drift.
