...
2. Apply the additive `drizzle/0015_prospecting_intelligence.sql` migration through the established Sites migration process after the earlier migrations. The build packages upward migrations only. `npm run db:generate` should report no drift.
...
**Do not enable live Google discovery until permitted retention and downstream CRM use are established for the applicable Google agreement.** The current design persists normalized listing facts and immutable evidence; its 24-hour freshness window is a cache decision, not legal permission to store Google content. Google's general Places policy restricts caching/storage with specific exceptions such as place IDs. If the applicable agreement does not permit this persistence, change the storage model before enabling discovery. Review [Google's Places policies](https://developers.google.com/maps/documentation/places/web-service/policies), attribution, public terms/privacy requirements, and any applicable EEA conditions.
