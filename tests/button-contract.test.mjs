...
test("shared button primitives expose the canonical variant and size contract", async () => {
  ...
  assert.equal((ui.match(/type = "button"/g) ?? []).length, 2);
  assert.equal((ui.match(/type=\{type\}/g) ?? []).length, 2);
  ...
  assert.match(workspace, /<Button[\s\S]*size="sm"[\s\S]*variant="ghost"[\s\S]*Más/);
