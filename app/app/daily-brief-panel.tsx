...
export function DailyBriefPanel({ canApprove, canCreate }: { canApprove: boolean; canCreate: boolean }) {
  ...
  return <section className="panel daily-brief-panel" aria-labelledby="daily-brief-title">
    <div className="panel-heading"><div><span className="eyebrow">HIDACA Daily Brief</span><h2 id="daily-brief-title">¿En qué debería enfocarme hoy?</h2></div><Button disabled={loading} onClick={() => void generate()} type="button" variant="secondary"><RefreshCw aria-hidden="true" size={15} /> {loading ? "Actualizando…" : "Actualizar"}</Button></div>
    {error && <div className="inline-alert" role="alert">{error}</div>}
    {!brief && !loading && <div className="daily-brief-empty"><Sparkles aria-hidden="true" size={22} /><p>Genera un brief con agenda, riesgos, cotizaciones, cobranza y proyectos usando datos reales del CRM.</p><Button onClick={() => void generate()} type="button">Generar Daily Brief</Button></div>}
    {brief && <>
