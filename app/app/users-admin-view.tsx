      ...
      <form className="user-editor" onSubmit={save}>
        ...
        {tab === "general" ? <div className="user-general-grid">
          ...
          <div className="permission-groups">{filteredModules.map((module) => {
            ...
            return <article key={module.key}><div className="permission-row"><span><small>{module.group}</small><strong>{module.label}</strong>{requiredBy.length > 0 && <em>Requerido por {requiredBy.join(", ")}</em>}{unmet && <em className="inline-warning">Prerequisito denegado: revisa {requiredBy.join(", ")} antes de autorizar este módulo.</em>}</span><label className="permission-toggle"><input checked={editor.permissions[module.key].view} className="choice-control" disabled={self || module.key === "usuarios" && editor.role !== "admin"} name={`permission-${module.key}-view`} onChange={(event) => setOverride(module.key, "view", event.target.checked ? "allow" : "deny")} type="checkbox" /> Acceso</label><button aria-expanded={expanded === module.key} onClick={() => setExpanded(expanded === module.key ? null : module.key)} type="button">CRUD</button></div>
